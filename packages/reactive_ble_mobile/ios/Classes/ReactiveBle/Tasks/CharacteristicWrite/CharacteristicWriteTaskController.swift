...
struct CharacteristicWriteTaskController: PeripheralTaskController {
    ...
    func start(peripheral: CBPeripheral) -> SubjectTask {
        guard
            ...
            let service = peripheral.services?.filter({ $0.uuid == task.key.serviceID })[Int(task.key.serviceIndex)],
            let characteristic = service.characteristics?.filter({ $0.uuid == task.key.id })[Int(task.key.index)],
